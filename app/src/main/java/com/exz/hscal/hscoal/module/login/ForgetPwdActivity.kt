...
class ForgetPwdActivity : BaseActivity(), View.OnClickListener {
    ...
    private fun resetTimer(b: Boolean, millisUntilFinished: Long) {
        if (b) {
            ...
            bt_code.text = String.format(mContext.getString(R.string.login_hint_get_reGetCode), millisUntilFinished / 1000)
    ...
    private fun checkRegister() {
        ...
        if (!StringUtil.isPhone(ed_phone.text.toString())) {
            ed_phone.setShakeAnimation()
            toast(getString(R.string.login_error_phone))
            return
        }
        if (TextUtils.isEmpty(ed_code.text.toString().trim())) {
        ...
        if (TextUtils.isEmpty(ed_pwd2.text.toString().trim())) {
            ed_pwd2.setShakeAnimation()
            toast(getString(R.string.forget_pwd_error_pwd2))
            return
        }
        if (!ed_pwd.text.toString().trim().equals(ed_pwd2.text.toString().trim())) {
            toast(getString(R.string.forget_pwd_pwd_no_equal))
            return
        }
        DataCtrlClass.forgetPwd(mContext, ed_phone.text.toString(), ed_code.text.toString(), ed_pwd.text.toString()) {
            if (it != null) {
                finish()
            }
        }
